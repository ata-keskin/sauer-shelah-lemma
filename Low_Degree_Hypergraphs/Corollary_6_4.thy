...
corollary corollary_6_4: 
  ...
  shows "measure_pmf.prob (vector_of (random_subset V p) n) {Ss. map_vector (\<lambda>S. card {u\<in>U. S u}) Ss = x \<and> (\<forall>S\<in>ran Ss. S \<sqsubseteq> V)} = (1 - p) ^ (d * n) * ((p * d)/(1-p))^(count x 1)"
