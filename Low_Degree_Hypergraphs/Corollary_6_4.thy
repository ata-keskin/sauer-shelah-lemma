...
corollary corollary_6_4: 
  assumes "0 \<le> p" and "p < 1" and "finite V" and "U \<subseteq> V" and 
          "card U = d" and "vector x" and "finite (dom x)" and "ran x \<subseteq> {0, 1}" and "len x = n"
  shows "measure_pmf.prob (pmf_vector (random_subset V p) n) {T \<in> \<llangle>power_set V\<rrangle>^n. map_vector ((\<lambda>_ S. card {u\<in>U. S u})) T = x} = (1 - p) ^ (d * n) * ((p * d)/(1-p))^(count x 1)"
...
corollary independent_events:
  assumes "0 \<le> p" and "p < 1" and "finite V" and "U \<subseteq> V" and "U' \<subseteq> V" and "card U = card U'"
          "U \<inter> U' = {}" and "vector x" and "finite (dom x)"  and "ran x \<subseteq> {0, 1}" and "len x = n"
...
proof (rule prob_space.indep_eventsI[OF measure_pmf.prob_space_axioms], simp)
  ...
  proof (cases "J = {U, U'}")
    case True
    with independence show ?thesis sledgehammer
  next
    case False
