...
lemma lemma_6_3:
  assumes "0 \<le> p" and p_less_1: "p < 1" and "finite V" and "U \<subseteq> V" and "U' \<subseteq> V" and 
          ...
          fin_vect_x: "x \<in> \<llangle>{0, 1}\<rrangle>^n"
  shows "measure_pmf.prob (pmf_vector (random_subset V p) n) {T \<in> \<llangle>power_set V\<rrangle>^n. map_vector ((\<lambda>_ S. card {u\<in>U. S u})) T = x \<and> map_vector ((\<lambda>_ S. card {u\<in>U'. S u})) T = x} = \<Phi> p d x r"
proof -
  ...
  have Pi_set_x_i_is_0: "{T_i \<in> power_set V. card {u\<in>U. T_i u} = 0 \<and> card {u\<in>U'. T_i u} = 0} = PiE_dflt V False (\<lambda>v. if v \<in> (U \<union> U') then {False} else UNIV)" (is "?lhs = ?rhs")
  proof
    show "?lhs \<subseteq> ?rhs"
    proof
      fix T_i assume "T_i \<in> ?lhs" hence a0: "card {u\<in>U. T_i u} = 0" and a1: "card {u\<in>U'. T_i u} = 0" and a2: "T_i \<in> power_set V" by blast+
      from a0 a1 finite_UnI[OF finite_U finite_U'] have a3: "\<forall>v\<in>V. v \<in> (U \<union> U') \<longrightarrow> \<not>T_i v" by auto
      from a2 have "\<forall>v\<in>UNIV - V. \<not>T_i v" unfolding power_set_def by blast
      with a3 show "T_i \<in> ?rhs" unfolding PiE_dflt_def by force
    qed
  next
    show "?rhs \<subseteq> ?lhs"
    proof
      fix T_i assume "T_i \<in> ?rhs" hence a0: "\<forall>v\<in>V. v \<in> (U \<union> U') \<longrightarrow> \<not>T_i v" and a1: "\<forall>v\<in>UNIV - V. \<not>T_i v" unfolding PiE_dflt_def by fastforce+
      from a0 assms(4,5) finite_U finite_U' have a2: "card {u\<in>U. T_i u} = 0" and a3: "card {u\<in>U'. T_i u} = 0" by auto
      from a1 a2 a3 show "T_i \<in> ?lhs" unfolding power_set_def by blast
    qed
  qed

  have inner: "i < n \<Longrightarrow> measure_pmf.prob (random_subset V p) {T_i \<in> power_set V. Some (card {u\<in>U. T_i u}) = x i \<and> Some (card {u\<in>U'. T_i u}) = x i} 
        = (1 - p) ^ (2 * d - r) * ((p * r)/(1-p) + ((p * (d-r))/(1-p))^2)^(x !. i)" for i
  proof (cases "x i")
    case None
    assume "i < n"
    with fin_vect_domI[OF fin_vect_x] None show ?thesis unfolding dom_def by blast
  next
    case (Some a)
    assume "i < n"
    ...
    proof (cases a)
      case 0
      from 0 have "measure_pmf.prob (random_subset V p) {T_i \<in> power_set V. Some (card {u\<in>U. T_i u}) = x i \<and> Some (card {u\<in>U'. T_i u}) = x i}
      =  measure_pmf.prob (random_subset V p) {T_i \<in> power_set V. card {u\<in>U. T_i u} = 0 \<and> card {u\<in>U'. T_i u} = 0}" using Some by force
      also have "... = measure_pmf.prob (Pi_pmf V False (\<lambda>_. bernoulli_pmf p)) {T_i \<in> power_set V. card {u\<in>U. T_i u} = 0 \<and> card {u\<in>U'. T_i u} = 0}" using Some by simp
    ...
    next
      ...
      with fin_vect_ranI[OF fin_vect_x] Some have a_is_1: "a = 1" unfolding ran_def by blast
      show ?thesis
      proof (cases "2 > 2 * d - r")
        case _: True
        ...
        proof
          ...
          proof
            ...
            have "measure_pmf.prob (random_subset V p) {T_i \<in> power_set V. Some (card {u\<in>U. T_i u}) = x i \<and> Some (card {u\<in>U'. T_i u}) = x i}
                           = measure_pmf.prob (Pi_pmf V False (\<lambda>_. bernoulli_pmf p)) {T_i \<in> power_set V. card {u\<in>U. T_i u} = 1 \<and> card {u\<in>U'. T_i u} = 1}" by (simp add: a_is_1 Some)
            also have "... = measure_pmf.prob (Pi_pmf V False (\<lambda>_. bernoulli_pmf p)) {T_i \<in> power_set V. card {u\<in>{}. T_i u} = 1 \<and> card {u\<in>{}. T_i u} = 1}" using U_empty U'_empty by fast
        ...
        next
          ...
          have Pi_set_inner_exception: "{T_i \<in> power_set V. card {u\<in>{v}. T_i u} = 1} = (PiE_dflt V False (\<lambda>u. if u \<in> {v} then {True} else UNIV))" (is "?lhs = ?rhs")
          proof
            show "?lhs \<subseteq> ?rhs"
            proof
              fix T_i assume "T_i \<in> ?lhs" hence a0: "card {u\<in>{v}. T_i u} = 1" and a1: "T_i \<in> power_set V" by blast+
              from a0 have a2: "T_i v" by (metis (no_types, lifting) a_noteq_0 a_is_1 card_eq_0_iff empty_Collect_eq singletonD)
              from a1 have "\<forall>v\<in>UNIV - V. \<not>T_i v" unfolding power_set_def by fastforce
              with a2 show "T_i \<in> ?rhs" unfolding PiE_dflt_def by simp
            qed
          next
            show "?rhs \<subseteq> ?lhs"
            proof
              ...
              with a1 show "S \<in> ?lhs" unfolding power_set_def by blast
            qed
          qed
            
          from a_is_1 have "measure_pmf.prob (random_subset V p) {T_i \<in> power_set V. Some (card {u\<in>U. T_i u}) = x i \<and> Some (card {u\<in>U'. T_i u}) = x i}
                           = measure_pmf.prob (Pi_pmf V False (\<lambda>_. bernoulli_pmf p)) {T_i \<in> power_set V. card {u\<in>{v}. T_i u} = 1}" using U_is_v U'_is_v Some by simp
      ...
      next
        ...
        let ?E1 = "{T_i \<in> power_set V. card {u\<in>(U \<inter> U'). T_i u} = 1 \<and> card {u\<in>(U \<union> U'). T_i u} = 1}"
        let ?E2 = "{T_i \<in> power_set V. card {u\<in>(U - U'). T_i u} = 1 \<and> card {u\<in>(U' - U). T_i u} = 1 \<and> card {u\<in>(U \<union> U'). T_i u} = 2}"
        have two_cases: "{T_i \<in> power_set V. card {u\<in>U. T_i u} = 1 \<and> card {u\<in>U'. T_i u} = 1} = ?E1 \<union> ?E2" (is "?lhs = ?E1 \<union> ?E2")
        proof
          show "?lhs \<subseteq> ?E1 \<union> ?E2"
          proof
            fix S assume "S \<in> ?lhs" hence asm1: "card {u\<in>U. S u} = 1" and asm2: "card {u\<in>U'. S u} = 1" and asm3: "S \<in> power_set V" by blast+
        ...
        next
          ...
          proof
            fix S assume "S \<in> ?E1 \<union> ?E2" hence asm1: "(card {u \<in> U \<inter> U'. S u} = 1 \<and> card {u \<in> U \<union> U'. S u} = 1) \<or> (card {u \<in> U - U'. S u} = 1 \<and> card {u \<in> U' - U. S u} = 1 \<and> card {u \<in> U \<union> U'. S u} = 2)" and asm2: "S \<in> power_set V" by blast+
        ...
        have Pi_set_E1: "u\<in>U \<inter> U' \<Longrightarrow> {T_i \<in> power_set V. T_i u \<and> card {u'\<in>(U \<union> U'). T_i u'} = 1} = PiE_dflt V False (\<lambda>v. (if v \<in> (U \<union> U') then {v = u} else UNIV))" (is "?asm \<Longrightarrow> ?lhs = ?rhs") for u 
        proof
          assume asm: ?asm
          hence u_in_Un: "u \<in> (U \<union> U')" by blast
          show "?lhs \<subseteq> ?rhs"
          proof
            fix S assume "S \<in> ?lhs" hence a0: "S u \<and> card {u'\<in>(U \<union> U'). S u'} = 1" and a1: "S \<in> power_set V" by blast+
            ...
            from a1 have "\<forall>v\<in>UNIV - V. \<not>S v" unfolding power_set_def by blast
        ...
        next
          ...
          proof
            ...
            from a1 have a3: "S \<in> power_set V" unfolding power_set_def by blast
            from a0 a2 u_in_Un have "{u'\<in>(U \<union> U'). S u'} = {u}" by blast
            with a3 show "S \<in> ?lhs" by force
          qed
        qed
  
        have case_1: "measure_pmf.prob (Pi_pmf V False (\<lambda>_. bernoulli_pmf p)) ?E1 = card (U \<inter> U') * p * (1 - p) ^ (card (U \<union> U') - 1)" (is "?lhs = ?rhs")
        proof -
          have E1_is: "?E1 = (\<Union>u\<in>U \<inter> U'. {S \<in> power_set V. S u \<and> card {u\<in>(U \<union> U'). S u} = 1})" (is "?lhs1 = ?rhs1")
          proof
            show "?lhs1 \<subseteq> ?rhs1"
            proof
              fix S assume "S \<in> ?lhs1" hence a: "card {u\<in>(U \<inter> U'). S u} = 1" and b: "card {u\<in>(U \<union> U'). S u} = 1" and c: "S \<in> power_set V" by blast+
          ...
          next
            ...
            proof
              fix S assume "S \<in> ?rhs1" then obtain u where a: "u \<in> U \<inter> U'" and b: "S u" and c: "card {u\<in>(U \<union> U'). S u} = 1" and d: "S \<in> power_set V" by blast+
          ...
          have p2: "(\<lambda>x. {S \<in> power_set V. S x \<and> card {u\<in>(U \<union> U'). S u} = 1}) ` (U \<inter> U') \<subseteq> measure_pmf.events (Pi_pmf V False (\<lambda>_. bernoulli_pmf p))" by force
          have p3: "disjoint_family_on (\<lambda>x. {S \<in> power_set V. S x \<and> card {u\<in>(U \<union> U'). S u} = 1}) (U \<inter> U')" unfolding disjoint_family_on_def
          proof (standard, standard, standard, standard, standard)
            fix m n S
            assume a1: "m \<in> U \<inter> U'" and a2: "n \<in> U \<inter> U'" and a3: "m \<noteq> n" and a4: "S \<in> {S \<in> power_set V. S m \<and> card {u \<in> U \<union> U'. S u} = 1} \<inter> {S \<in> power_set V. S n \<and> card {u \<in> U \<union> U'. S u} = 1}"
          ...
          from E1_is have "?lhs = measure_pmf.prob (Pi_pmf V False (\<lambda>_. bernoulli_pmf p)) (\<Union>u\<in>U \<inter> U'. {S \<in> power_set V. S u \<and> card {u\<in>(U \<union> U'). S u} = 1})" by presburger
          also from measure_pmf.finite_measure_finite_Union[OF p1 p2 p3] have "... = (\<Sum>u\<in>(U \<inter> U'). measure_pmf.prob (Pi_pmf V False (\<lambda>_. bernoulli_pmf p)) {S \<in> power_set V. S u \<and> card {u'\<in>(U \<union> U'). S u'} = 1})" .
        ...
        have Pi_set_E2: "\<lbrakk>u \<in> U - U'; u' \<in> U' - U\<rbrakk> \<Longrightarrow> {S \<in> power_set V. S u \<and> S u' \<and> card {u\<in>(U \<union> U'). S u} = 2} = (PiE_dflt V False (\<lambda>v. (if v \<in> (U \<union> U') then {v = u \<or> v = u'} else UNIV)))" (is "\<lbrakk>?asm1; ?asm2\<rbrakk> \<Longrightarrow> ?lhs = ?rhs") for u u'
        proof
          assume asm1: ?asm1 and asm2: ?asm2
          show "?lhs \<subseteq> ?rhs"
          proof
            fix S assume "S \<in> ?lhs" hence a0: "S u" and a1: "S u'" and a2: "card {u \<in> U \<union> U'. S u} = 2" and a3: "S \<in> power_set V" by blast+
            ...
            from a3 have "\<forall>v\<in>UNIV - V. \<not>S v" unfolding power_set_def by blast
        ...
        next
          ...
          proof
            ...
            from a1 have a6: "S \<in> power_set V" unfolding power_set_def by blast
        ...
        proof -
          ...
          have E2_is: "?E2 = (\<Union>u\<in>U - U'. \<Union>u'\<in>U'- U. {S \<in> power_set V. S u \<and> S u' \<and> card {u\<in>(U \<union> U'). S u} = 2})" (is "?lhs1 = ?rhs1")
          proof
            show "?lhs1 \<subseteq> ?rhs1"
            proof
              fix S assume "S \<in> ?lhs1" hence a0: "card {u \<in> U - U'. S u} = 1" and a1: "card {u \<in> U' - U. S u} = 1" and a2: "card {u \<in> U \<union> U'. S u} = 2" and a3: "S \<in> power_set V" by blast+
          ...
          next
            show "?rhs1 \<subseteq> ?lhs1"
            proof
              fix S assume "S \<in> ?rhs1" then obtain u u' where a0: "u \<in> U - U'" and a1: "u' \<in> U' - U" and a2: "S \<in> {S \<in> power_set V. S u \<and> S u' \<and> card {u \<in> U \<union> U'. S u} = 2}" by blast
              from a2 have a3: "S u" and a4: "S u'" and a5: "card {u \<in> U \<union> U'. S u} = 2" and a6: "S \<in> power_set V" by blast+
          ...
          have p2: "(\<lambda>x. (\<Union>u'\<in>U'- U. {S \<in> power_set V. S x \<and> S u' \<and> card {u\<in>(U \<union> U'). S u} = 2})) ` (U - U') \<subseteq> measure_pmf.events (Pi_pmf V False (\<lambda>_. bernoulli_pmf p))" by force
          have p3: "disjoint_family_on (\<lambda>x.  (\<Union>u'\<in>U' - U. {S \<in> power_set V. S x \<and> S u' \<and> card {u\<in>(U \<union> U'). S u} = 2})) (U - U')" unfolding disjoint_family_on_def
          proof (standard, standard, standard, standard, standard)
            fix m n S
            assume a1: "m \<in> U - U'" and a2: "n \<in> U - U'" and a3: "m \<noteq> n" and a4: "S \<in> (\<Union>u'\<in>U'- U. {S \<in> power_set V. S m \<and> S u' \<and> card {u\<in>(U \<union> U'). S u} = 2}) \<inter> (\<Union>u'\<in>U' - U. {S \<in> power_set V. S n \<and> S u' \<and> card {u\<in>(U \<union> U'). S u} = 2})"
            from a4 have "\<exists>u'\<in>U' - U. S \<in> {S \<in> power_set V. S m \<and> S n \<and> S u' \<and> card {u\<in>(U \<union> U'). S u} = 2}" by blast
          ...
          have p5: "y \<in> (U - U') \<Longrightarrow> (\<lambda>z. {S \<in> power_set V. S y \<and> S z \<and> card {u\<in>(U \<union> U'). S u} = 2}) ` (U' - U) \<subseteq> measure_pmf.events (Pi_pmf V False (\<lambda>_. bernoulli_pmf p))" for y by force
          have p6: "y \<in> (U - U') \<Longrightarrow> disjoint_family_on (\<lambda>z. {S \<in> power_set V. S y \<and> S z \<and> card {u\<in>(U \<union> U'). S u} = 2}) (U' - U)" for y unfolding disjoint_family_on_def
          proof (standard, standard, standard, standard, standard)
            fix m n S
            assume a1: "y \<in> (U - U')" and a2: "m \<in> U' - U" and a3: "n \<in> U' - U" and a4: "m \<noteq> n" and a5: "S \<in> {S \<in> power_set V. S y \<and> S m \<and> card {u\<in>(U \<union> U'). S u} = 2} \<inter> {S \<in> power_set V. S y \<and> S n \<and> card {u\<in>(U \<union> U'). S u} = 2}"
          ...
          from E2_is have "?lhs = measure_pmf.prob (Pi_pmf V False (\<lambda>_. bernoulli_pmf p)) (\<Union>u\<in>U - U'. \<Union>u'\<in>U'- U. {S \<in> power_set V. S u \<and> S u' \<and> card {u\<in>(U \<union> U'). S u} = 2})" by presburger
          also from measure_pmf.finite_measure_finite_Union[OF p1 p2 p3] have "... = (\<Sum>u\<in>(U - U'). measure_pmf.prob (Pi_pmf V False (\<lambda>_. bernoulli_pmf p)) (\<Union>u'\<in>U'- U. {S \<in> power_set V. S u \<and> S u' \<and> card {u\<in>(U \<union> U'). S u} = 2}))" .
          also from measure_pmf.finite_measure_finite_Union[OF p4 p5 p6] have "... = (\<Sum>u\<in>(U - U'). \<Sum>u'\<in>(U'- U). measure_pmf.prob (Pi_pmf V False (\<lambda>_. bernoulli_pmf p)) {S \<in> power_set V. S u \<and> S u' \<and> card {u\<in>(U \<union> U'). S u} = 2})" by fastforce
        ...
        from a_is_1 have "measure_pmf.prob (random_subset V p) {S \<in> power_set V. Some (card {u\<in>U. S u}) = x i \<and> Some (card {u\<in>U'. S u}) = x i}
                   = measure_pmf.prob (Pi_pmf V False (\<lambda>_. bernoulli_pmf p)) {S \<in> power_set V. card {u\<in>U. S u} = 1 \<and> card {u\<in>U'. S u} = 1}" using Some by simp
  ...
  have "measure_pmf.prob (pmf_vector (random_subset V p) n) {T \<in> \<llangle>power_set V\<rrangle>^n. map_vector (\<lambda>_ S. card {u\<in>U. S u}) T = x \<and> map_vector (\<lambda>_ S. card {u\<in>U'. S u}) T = x} 
    = (\<Prod>i | i < n. measure_pmf.prob (random_subset V p) {T_i \<in> power_set V. Some (card {u\<in>U. T_i u}) = x i \<and> Some (card {u\<in>U'. T_i u}) = x i})" 
    using measure_Pi_pmf_vector[OF fin_vect_x, of "random_subset V p" "power_set V" "{(\<lambda>_ S. card {u \<in> U. S u}), (\<lambda>_ S. card {u \<in> U'. S u})}"] by force
  also have "... = (\<Prod>i | i < n. (1 - p) ^ (2 * d - r) * ((p * r)/(1-p) + (p * (d-r)/(1-p))^2)^(x !. i))" using inner by fastforce
  also have "... = (\<Prod>i | i < n. (1 - p) ^ (2 * d - r)) * ((\<Prod>i\<in>{i. i<n}. ((p * r)/(1-p) + ((p * (d-r))/(1-p))^2)^(x !. i)))" by (simp only: prod.distrib)
